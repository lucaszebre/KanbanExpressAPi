...
export const updateTaskSubtask = async (req, res) => {
    ...
    try {
        ...
        if (subtasksChangeName && subtasksChangeName.length > 0) {
            const updateSubtasksPromises = subtasksChangeName.map((subtask) => prisma.subtask.update({
                where: { id: subtask.id },
        ...
        if (subtasksAdd && subtasksAdd.length > 0) {
            const addSubtasksPromises = subtasksAdd.map((title) => prisma.subtask.create({
                data: {
                    title,
                    task: { connect: { id } },
                },
